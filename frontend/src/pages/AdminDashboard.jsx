...
function AdminSettings({ user: sessionUser, onUserChange }) {
  ...
  return <div className="grid items-start gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
    <aside className="space-y-6 xl:sticky xl:top-4">
      ...
      <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_12px_30px_rgba(43,37,80,.06)]"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#6F52D9]">Account details</p><div className="mt-5 space-y-4 text-sm"><div className="flex items-center justify-between gap-3"><span className="text-slate-400">Access level</span><span className="font-bold capitalize text-slate-700">{profile.role}</span></div><div className="flex items-center justify-between gap-3"><span className="text-slate-400">Sign-in method</span><span className="font-bold capitalize text-slate-700">{profile.auth_provider}</span></div><div className="flex items-center justify-between gap-3"><span className="text-slate-400">Email status</span><span className={`font-bold ${profile.is_email_verified ? "text-emerald-600" : "text-amber-600"}`}>{profile.is_email_verified ? "Verified" : "Not verified"}</span></div><div className="border-t border-slate-100 pt-4"><p className="text-slate-400">Administrator since</p><p className="mt-1 font-bold text-slate-700">{new Date(profile.created_at).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</p></div></div></section>
    </aside>
