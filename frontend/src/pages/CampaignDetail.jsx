...
function SectionTitle({ eyebrow, title, description, icon: Icon }) {
  return (
    <div className="flex items-end justify-between gap-4">
      <div>
        <p className="text-xs font-bold uppercase tracking-[.18em] text-[#7451E8]">
          {eyebrow}
        </p>
...
export default function CampaignDetail() {
  ...
  useEffect(() => {
    loadCampaign().catch(() => setError("This campaign could not be found."));
  }, [id]);
  useEffect(() => {
    ...
    const tick = () =>
      setSecondsRemaining(
        Math.max(
          0,
          Math.ceil(
            (new Date(checkout.expires_at).getTime() - Date.now()) / 1000,
          ),
        ),
      );
    tick();
  ...
  const publishUpdate = async (event) => {
    ...
    if (!updateTitle.trim() || !updateBody.trim())
      return setUpdateError("Add a title and message before publishing.");
    setPublishingUpdate(true);
    try {
      const { data } = await api.post(`/campaigns/${id}/updates/`, {
      ...
      setUpdateError(
        requestError.response?.data?.detail || "Could not publish this update.",
      );
    } finally {
  ...
  const startDonation = async (event) => {
    ...
    if (!numeric || numeric < 1000)
      return setDonationMessage("Enter at least 1,000 Ks.");
    setDonating(true);
    try {
      ...
      setDonationMessage(
        requestError.response?.data?.detail ||
          "Could not start the demo payment.",
      );
    } finally {
      setDonating(false);
    }
  ...
  const simulatePayment = async (outcome) => {
    ...
    try {
      ...
      if (outcome !== "success") {
        setCheckout(null);
        return setDonationMessage(
          data.failure_reason || "Demo payment did not complete.",
        );
      }
      await loadCampaign();
      setReceipt(data);
  ...
  const submitUtilization = async (event) => {
    ...
    setUtilizationError("");
    if (
      !utilizationForm.title ||
      !utilizationForm.description ||
      !utilizationForm.amount_spent ||
      !utilizationForm.spent_on
    )
      return setUtilizationError(
        "Complete the title, explanation, amount, and date.",
      );
    const form = new FormData();
    Object.entries(utilizationForm).forEach(([key, value]) => {
    ...
    try {
      ...
      setUtilizationError(
        requestError.response?.data?.detail || "Could not publish this report.",
      );
    } finally {
  ...
  if (error)
    return (
      <div className="grid min-h-screen place-items-center bg-[#F8F7FC] p-6 text-center">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900">{error}</h1>
          <Link
            to="/campaigns"
            className="mt-5 inline-block font-bold text-[#7451E8]"
          >
  ...
  if (!campaign)
    return (
      <div className="grid min-h-screen place-items-center bg-[#F8F7FC]">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-[#E6DEFF] border-t-[#7451E8]" />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8F7FC] text-slate-900">
      <main className="mx-auto max-w-[1280px] px-4 py-7 sm:px-6 lg:py-10">
        <Link
          to="/campaigns"
          className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-[#7451E8]"
        >
            ...
            <div className="flex flex-col p-7 sm:p-9">
              ...
              <div className="mt-8">
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-4xl font-extrabold text-[#FFD66B]">
                      {kyat(campaign.amount_raised)}
                    </p>
                    <p className="mt-1 text-sm text-indigo-200">
                      of {kyat(campaign.goal_amount)} goal
                    </p>
                  </div>
                ...
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-white/15">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[#FFD66B] to-[#FFAC6A]"
              ...
              <p className="mt-auto pt-6 text-sm text-indigo-100">
                Organized by{" "}
                <strong className="text-white">{campaign.owner_name}</strong>{" "}
                for{" "}
                <strong className="text-white">{campaign.beneficiary}</strong>.
              </p>
            </div>
          </div>
        </section>
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_380px]">
          <div className="space-y-10">
            <section className="rounded-3xl bg-white p-7 shadow-[0_12px_32px_rgba(41,35,74,.06)]">
              ...
              />
              <p className="mt-6 whitespace-pre-wrap text-[15px] leading-8 text-slate-600">
                {campaign.story}
              </p>
            </section>
            <section className="rounded-3xl bg-white p-7 shadow-[0_12px_32px_rgba(41,35,74,.06)]">
              <SectionTitle
                ...
                description="Follow the work being done and the difference your support makes."
                icon={MessageCircleHeart}
              />
              {isOrganizer && (
                <form
                  onSubmit={publishUpdate}
                  className="mt-7 rounded-2xl bg-[#F6F3FF] p-5"
                >
                  <p className="flex items-center gap-2 text-sm font-extrabold text-[#6549C9]">
                    <Sparkles size={17} /> Publish an update
                  </p>
                  <input
                    value={updateTitle}
                    ...
                    className="mt-4 w-full rounded-xl border border-[#DED4FF] bg-white px-4 py-3 text-sm outline-none focus:border-[#7451E8]"
                  />
                  <textarea
                    value={updateBody}
                    ...
                    maxLength={2000}
                    rows={3}
                    placeholder="Tell supporters what has happened."
                    className="mt-3 w-full rounded-xl border border-[#DED4FF] bg-white px-4 py-3 text-sm outline-none focus:border-[#7451E8]"
                  />
                  {updateError && (
                  ...
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[#7451E8] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"
                  >
                    <Send size={16} />{" "}
              ...
              <div className="mt-7 space-y-6">
                {updates.length ? (
                  updates.map((item) => (
                    <article
                      key={item.id}
                      className="relative border-l-2 border-[#D9CEFF] pl-6"
                    >
            ...
            <section className="rounded-3xl bg-white p-7 shadow-[0_12px_32px_rgba(41,35,74,.06)]">
              <SectionTitle
                ...
                title="How the funds are used"
                description="Givera administrators publish spending reports and supporting evidence."
                icon={ReceiptText}
              />
              {isAdmin && (
                <form
                  onSubmit={submitUtilization}
                  className="mt-7 rounded-2xl border border-[#DDD3FF] bg-[#F6F3FF] p-5"
                >
                  <p className="text-sm font-extrabold text-[#6549C9]">
                    Publish a spending report
                  </p>
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <input
                      value={utilizationForm.title}
                      onChange={(event) =>
                        setUtilizationForm((form) => ({
                          ...form,
                          title: event.target.value,
                        }))
                      }
                      placeholder="Purchase or expense"
                      className="rounded-xl border border-[#DED4FF] bg-white px-3 py-3 text-sm outline-none"
                    />
                    <input
                      type="number"
                      min="1"
                      value={utilizationForm.amount_spent}
                      onChange={(event) =>
                        setUtilizationForm((form) => ({
                          ...form,
                    ...
                    <input
                      type="date"
                      value={utilizationForm.spent_on}
                      onChange={(event) =>
                        setUtilizationForm((form) => ({
                          ...form,
                    ...
                    <input
                      ...
                      className="rounded-xl border border-[#DED4FF] bg-white px-2 py-2 text-xs"
                    />
                  </div>
                  <textarea
                    ...
                    rows={3}
                    placeholder="Explain this expense and its impact."
                    className="mt-3 w-full rounded-xl border border-[#DED4FF] bg-white px-3 py-3 text-sm outline-none"
                  />
                  {utilizationError && (
                    <p className="mt-3 text-sm font-medium text-rose-600">
                      {utilizationError}
                  ...
                  <button
                    disabled={submittingUtilization}
                    className="mt-3 rounded-xl bg-[#7451E8] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"
                  >
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs font-bold uppercase tracking-wide text-[#7451E8]">
                            {kyat(item.amount_spent)} ·{" "}
                      ...
                      {item.evidence && (
                        <a
                          href={mediaUrl(item.evidence)}
                          target="_blank"
                          rel="noreferrer"
                          className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-[#7451E8]"
                        >
          ...
          <aside className="lg:sticky lg:top-6 lg:self-start">
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_15px_35px_rgba(41,35,74,.08)]">
              <p className="text-xs font-bold uppercase tracking-[.16em] text-[#7451E8]">
                Support this cause
              </p>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Choose an amount and experience the secure Myanmar wallet
                checkout demo.
              </p>
              {isLoggedIn ? (
                <form onSubmit={startDonation} className="mt-6">
                  ...
                  <div className="mt-2 flex overflow-hidden rounded-xl border border-slate-200 focus-within:border-[#7451E8]">
                    <span className="grid w-14 place-items-center border-r border-slate-200 bg-slate-50 text-xs font-extrabold text-[#7451E8]">
                      Ks
                    </span>
                    <input
                      type="number"
                      ...
                      placeholder="10000"
                      className="min-w-0 flex-1 px-4 py-3 font-bold outline-none"
                    />
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {[5000, 10000, 20000, 50000].map((preset) => (
                      <button
                        ...
                        className="rounded-xl bg-slate-50 px-2 py-2.5 text-xs font-bold text-[#6549C9] hover:bg-[#F1EDFF]"
                      >
                        {kyat(preset)}
                      </button>
                    ))}
                  ...
                  <label className="mt-5 block text-sm font-bold">
                    ...
                    <textarea
                      ...
                      className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-normal outline-none focus:border-[#7451E8]"
                    />
                  </label>
                  ...
                  <fieldset className="mt-5">
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      {[
                        ...
                        <label
                          key={value}
                          className={`cursor-pointer rounded-xl border px-2 py-2.5 text-center text-xs font-extrabold ${provider === value ? "border-[#7451E8] bg-[#F1EDFF] text-[#6549C9]" : "border-slate-200 text-slate-500"}`}
                        >
                          <input
                            type="radio"
                            value={value}
                  ...
                  <button
                    disabled={donating}
                    className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#7451E8] px-4 py-4 font-bold text-white shadow-lg shadow-violet-300/40 disabled:opacity-60"
                  >
                    <Heart size={18} />{" "}
                    {donating ? "Opening checkout…" : "Continue to payment"}
                  </button>
                  <p className="mt-3 text-center text-xs leading-5 text-slate-400">
                    Demo only. No wallet, QR code, or money is connected.
                  </p>
                </form>
              ) : (
                <div className="mt-6">
                  <Link
                    to="/login"
                    className="flex items-center justify-center gap-2 rounded-xl bg-[#7451E8] px-4 py-4 font-bold text-white"
                  >
            ...
            <div className="mt-5 rounded-3xl bg-[#211842] p-6 text-white">
              ...
              <p className="mt-2 text-sm leading-6 text-indigo-100">
                See the supporters and every administrator-published fund
      ...
      {checkout && (
        <Checkout
          ...
          onPay={() => simulatePayment("success")}
          onFail={() => simulatePayment("failed")}
          onCancel={() => simulatePayment("cancelled")}
        />
      )}
      {receipt && (
...
function Empty({ label }) {
  return (
    <div className="mt-6 rounded-2xl bg-slate-50 px-5 py-8 text-center text-sm text-slate-400">
      {label}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#17112E]/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-[28px] bg-white shadow-2xl">
        <div className="flex items-start justify-between bg-[#25194B] px-6 py-5 text-white">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.18em] text-[#D7C8FF]">
              Sandbox checkout
            </p>
        ...
        <div className="p-6 text-center">
          <div className="mx-auto grid h-24 w-24 grid-cols-3 gap-1 rounded-2xl bg-[#F1EDFF] p-3">
            {Array.from({ length: 9 }, (_, index) => (
              <span
                key={index}
                className={`rounded-sm ${[0, 1, 3, 4, 8].includes(index) ? "bg-[#7451E8]" : "bg-[#CFC2FF]"}`}
              />
            ))}
          </div>
          <p className="mt-5 text-3xl font-extrabold">
            {kyat(checkout.amount)}
          </p>
          ...
          <div className="mt-5 rounded-xl bg-slate-50 px-4 py-3 text-left text-xs">
            ...
            <p className="mt-1 font-mono font-extrabold text-slate-700">
              {checkout.transaction_reference}
            </p>
          </div>
          <p
            className={`mt-3 text-xs font-bold ${seconds ? "text-slate-500" : "text-rose-600"}`}
          >
            {seconds
              ? `Session expires in ${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`
              : "Demo session expired"}
          </p>
          <div className="mt-5 rounded-2xl bg-amber-50 p-4 text-left text-sm text-amber-900">
            <p className="flex items-center gap-2 font-extrabold">
              <ShieldCheck size={18} /> No real payment
            </p>
            <p className="mt-1 leading-5">
              This is a non-scannable demonstration of the wallet return
              process.
            </p>
          </div>
          <button
            ...
            className="mt-5 w-full rounded-xl bg-[#7451E8] px-4 py-3.5 font-bold text-white disabled:opacity-50"
          >
            <CheckCircle2 className="mr-2 inline" size={18} />{" "}
            {busy ? "Completing…" : "Simulate successful payment"}
          </button>
          <div className="mt-3 grid grid-cols-2 gap-3">
            <button
              ...
              onClick={onFail}
              disabled={busy || !seconds}
              className="rounded-xl bg-rose-50 px-3 py-2.5 text-xs font-bold text-rose-700 disabled:opacity-50"
            >
              Simulate failure
            </button>
            <button
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#17112E]/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] bg-white p-7 text-center shadow-2xl">
        ...
        <p className="mt-5 text-xs font-bold uppercase tracking-[.18em] text-[#7451E8]">
          Demo receipt
        </p>
        ...
        <p className="mt-5 text-xs text-slate-400">
          DEMO PAYMENT — no funds were transferred.
        </p>
        <button
          type="button"
          onClick={onClose}
          className="mt-5 w-full rounded-xl bg-[#7451E8] px-4 py-3 font-bold text-white"
        >
          Done
        </button>
      </div>
